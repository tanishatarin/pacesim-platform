...
export const usePacemakerData = (): PacemakerDataHook => {
  ...
  useEffect(() => {
    ...
    const connectionUnsubscribe = sharedClient.onConnectionStatus(
      (connected) => {
        ...
        if (connected) {
          ...
          setErrorMessage(
            "Connection lost. Will switch to simulation in 3 seconds...",
          );
          ...
          if (wasConnected && lastStateRef.current) {
            disconnectTimeoutRef.current = setTimeout(() => {
              console.log(
                "📱 Switching to simulation mode with last known values",
              );
  ...
  const sendControlUpdate = (updates: Partial<PacemakerState>) => {
    ...
    try {
      ...
      setErrorMessage(
        `Error sending update: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
