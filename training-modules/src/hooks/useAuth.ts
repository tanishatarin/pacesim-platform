...
export const useAuth = () => {
  ...
  const login = async (username: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      const existingUser = db.data.users.find(u => u.email === username || u.name === username)
  ...
  const signup = async (username: string, password: string, name?: string, role?: string, institution?: string): Promise<{ success: boolean; error?: string }> => {
    try {
      db.read()
