...
export const useSession = (userId?: string) => {
  ...
  const addParameterChange = useCallback((sessionId: string, parameter: string, oldValue: number, newValue: number, reason?: string) => {
    if (!userId || !currentSession) return;
  ...
  const incrementHints = useCallback((sessionId: string) => {
  ...
  const incrementErrors = useCallback((sessionId: string) => {
    if (!currentSession) return;
  ...
  const getSessionStats = useCallback(() => {
    ...
    return {
      ...
      completionRate: userSessions.length > 0 ? (completedSessions.length / userSessions.length) * 100 : 0,
      averageScore: completedSessions.length > 0 
        ? completedSessions.reduce((sum, s) => sum + (s.quizState?.score || 0), 0) / completedSessions.length 
        : 0,
      totalTimeSpent: userSessions.reduce((sum, s) => sum + (s.totalTimeSpent || 0), 0),
      averageParameterChanges: userSessions.length > 0
        ? userSessions.reduce((sum, s) => sum + (s.practiceState?.parameterChanges?.length || 0), 0) / userSessions.length
        : 0
    };
  }, [userId, sessionHistory]);
  ...
  const exportSessionData = useCallback(() => {
    ...
    return {
      ...
      moduleProgress: db.data?.moduleProgress?.filter(p => p.userId === userId) || [],
