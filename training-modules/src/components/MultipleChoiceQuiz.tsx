...
//       ],
//       correctAnswer: 1,
//       explanation: 'Decreasing sensitivity makes the pacemaker less sensitive to small signals that may cause inappropriate inhibition.'
//     }
//   ],
//   3: [
//     {
...
//       ],
//       correctAnswer: 3,
//       explanation: 'All of these factors can contribute to undersensing by affecting the pacemaker\'s ability to detect intrinsic cardiac activity.'
//     }
//   ],
//   4: [
//     {
...
//     {
//       id: 'capture_q2',
//       question: 'What is the typical starting point for capture threshold testing?',
//       options: [
//         '1.0 mA',
//         '5.0 mA',
//         '10.0 mA',
...
//       ],
//       correctAnswer: 1,
//       explanation: 'Capture threshold testing typically starts at 5.0 mA and is gradually decreased until loss of capture occurs.'
//     }
//   ],
//   5: [
//     {
...
//       ],
//       correctAnswer: 3,
//       explanation: 'All of these factors can prevent the pacemaker from successfully capturing and stimulating the heart muscle.'
//     }
//   ]
// };
...
//   const { currentSession, updateSession, startSession } = useSession(currentUser?.id);
...
//   // ✅ FIXED: Smart session management - only restore if session exists
//   useEffect(() => {
//     if (!currentUser || questions.length === 0) return;

//     console.log('🎓 Quiz session check:', {
//       hasCurrentSession: !!currentSession,
//       sessionStep: currentSession?.currentStep,
//       isQuizCompleted: currentSession?.quizState?.isCompleted
//     });

//     // If we have a session, restore its state
//     if (currentSession?.quizState) {
//       const { quizState } = currentSession;
      
//       console.log('📚 Restoring quiz state from session:', quizState);
      
//       if (quizState.isCompleted) {
//         // Quiz was completed - show review mode
...
//       } else {
//         // Quiz in progress - restore partial state
...
//           if (question) {
//             restoredAnswers[question.id] = answer.selectedAnswer;
//             // Only mark as revealed if the answer has been submitted (has isCorrect defined)
...
//         console.log('🔄 Restored partial quiz progress:', {
//           currentQuestionIndex: quizState.currentQuestionIndex,
//           answersCount: quizState.answers.length,
//           revealedCount: Object.keys(restoredRevealed).length
//         });
//       }
//     }
//   }, [currentSession?.id, questions.length, currentUser]);
...
//   // ✅ NEW: Function to create session when user first interacts with quiz
//   const ensureSessionExists = useCallback(() => {
//     if (!currentSession && currentUser) {
//       console.log('🆕 Creating new session for quiz interaction');
//       const moduleTitle = `Module ${moduleId}`;
//       const sessionId = startSession(moduleId.toString(), moduleTitle);
//       return sessionId;
//     }
//     return currentSession?.id || null;
//   }, [currentSession, currentUser, moduleId, startSession]);

//   // ✅ FIXED: Only save to session if one exists, create when needed
...
//     // ✅ SAVE TO SESSION: Create session if needed, then save
//     if (currentSession) {
//       console.log('💾 Saving answer selection to existing session:', { 
...
//       const existingAnswerIndex = existingAnswers.findIndex(a => a.questionIndex === currentQuestionIndex);
...
//         };
//       } else {
//         // Add new answer (not yet marked correct/incorrect)
//         updatedAnswers = [...existingAnswers, {
//           questionIndex: currentQuestionIndex,
//           selectedAnswer: answerIndex,
//           isCorrect: false, // Will be updated when revealed
//           timestamp: new Date().toISOString()
//         }];
//       }
...
//       });
//     } else {
//       console.log('📝 No session available - answer selection not saved (will be saved when session created)');
//     }
//   }, [currentQuestion.id, questionsRevealed, showReviewMode, currentSession, currentQuestionIndex, updateSession]);

//   // ✅ UPDATED: Submit answer with session creation if needed
...
//     // Ensure session exists before saving answer
//     const sessionId = ensureSessionExists();
//     if (!sessionId) {
//       console.error('❌ Could not create or find session for answer submission');
//       return;
//     }
...
//     console.log('✅ Submitting answer:', { questionIndex: currentQuestionIndex, selectedAnswer, isCorrect });
    
//     // Get current session (might be newly created)
//     const session = currentSession || { quizState: { answers: [] } };
//     const existingAnswers = session.quizState.answers || [];
//     const existingAnswerIndex = existingAnswers.findIndex(a => a.questionIndex === currentQuestionIndex);
    
//     let updatedAnswers;
//     if (existingAnswerIndex >= 0) {
//       // Update existing answer with correctness
//       updatedAnswers = [...existingAnswers];
//       updatedAnswers[existingAnswerIndex] = {
//         ...updatedAnswers[existingAnswerIndex],
//         selectedAnswer,
...
//       };
//     } else {
//       // Add new answer with correctness
//       updatedAnswers = [...existingAnswers, {
//         questionIndex: currentQuestionIndex,
//         selectedAnswer,
//         isCorrect,
...
//     }));
//   }, [currentQuestion, selectedAnswers, currentSession, currentQuestionIndex, updateSession, ensureSessionExists, questions.length]);
...
//       // ✅ SAVE PROGRESS: Update current question index in session
//       if (currentSession) {
...
//       completeQuiz();
//     }
//   }, [currentQuestionIndex, questions.length, quizCompleted, currentSession, updateSession]);
...
//       // ✅ SAVE PROGRESS: Update current question index in session
//       if (currentSession) {
//         console.log('⬅️  Moving to question:', prevIndex);
//         updateSession(currentSession.id, {
//           quizState: {
//             ...currentSession.quizState,
...
//     // ✅ SAVE FINAL QUIZ STATE: Mark quiz as completed in session
//     if (currentSession) {
//       console.log('🎓 Completing quiz:', { correctAnswers, total: questions.length, passed });
//       updateSession(currentSession.id, {
...
//               Session: {currentSession.quizState.answers.length} saved
//             </div>
//           )}
//         </div>
...
//       {currentSession && !quizCompleted && (
//         <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-6">
//           <p className="text-gray-700 text-sm">
//             💾 Your progress is automatically saved. You can safely close and resume this session later.
//             <span className="ml-2 text-blue-600">
//               ({currentSession.quizState.answers.length}/{questions.length} answers saved)
//             </span>
//           </p>
//         </div>
//       )}
...
//                 Session saved - you can resume this module anytime from where you left off.
//               </p>
//             )}
//           </div>
//         </div>
...
const MultipleChoiceQuiz = ({ moduleId, onComplete, className = '' }: MultipleChoiceQuizProps) => {
  ...
  const handleAnswerSelect = useCallback((answerIndex: number) => {
    ...
    if (currentSession) {
      const existingAnswers = currentSession.quizState.answers || [];
      const existingIndex = existingAnswers.findIndex(a => a.questionIndex === currentQuestionIndex);
  ...
  }, [currentQuestion, selectedAnswers, currentSession, currentQuestionIndex, updateSession]);
  ...
  return (
    <div className={`bg-white rounded-xl border border-gray-200 p-6 ${className}`}>
      ...
      {/* Auto-save message - only show if session exists and quiz not completed */}
      {currentSession && !quizCompleted && currentSession.quizState.answers.length > 0 && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-6">
          <p className="text-gray-700 text-sm">
            💾 Your progress is automatically saved. You can safely close and resume this session later.
            <span className="ml-2 text-blue-600">
