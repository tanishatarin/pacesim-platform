...
const ModulePage = () => {
  ...
  const { startSession, endSession, currentSession } = useSession(currentUser?.id);
  ...
  const [patientHeartRate] = useState(40); // This stays constant as the patient's baseline
  ...
  const handleQuizComplete = useCallback((passed: boolean, score: number, totalQuestions: number) => {
    ...
    // Start session when quiz is completed
    if (currentUser && !currentSession) {
      startSession(moduleId || '1', currentModule.title);
    }
    
    console.log('Quiz completed:', { passed, score, totalQuestions, moduleId });
  }, [moduleId, currentUser, currentSession, startSession, currentModule.title]);

  const handleComplete = useCallback((success: boolean) => {
    const sessionDuration = Math.floor((Date.now() - sessionStartTime) / 1000);
    
    // End session with database tracking
    if (currentSession) {
      const finalScore = success ? Math.round((quizScore.score / quizScore.total) * 100) : 0;
      endSession(success, finalScore, quizScore.total);
    }
  ...
  }, [sessionStartTime, moduleId, quizPassed, quizScore, pacemakerParams, connectionMode, currentSession, endSession]);
      ...
      <div className="w-full px-8 py-8 bg-white shadow-lg rounded-3xl">
        ...
        <div className="grid grid-cols-3 gap-8">
          ...
          <div className="col-span-2 space-y-6">
            ...
            {/* Simulation Controls - NOW POSITIONED HERE for better UX */}
            ...
            {/* Quiz Section - Enhanced to allow navigation after completion */}
            <div className="space-y-2">
          ...
          <div className="space-y-6">
            ...
            {/* Patient's Intrinsic Heart Rate - separate from pacemaker */}
            <div className="bg-[#F0F6FE] rounded-xl p-4">
            ...
            {/* Pacemaker Rate - now separate from patient HR */}
            <div className="bg-[#F0F6FE] rounded-xl p-4">
            ...
            {/* Action Buttons - Always available after quiz completion */}
            {quizCompleted && (
