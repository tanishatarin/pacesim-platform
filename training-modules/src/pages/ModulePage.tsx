...
const ModulePage = () => {
  ...
  // Connection mode management with auto-fallback
  const [connectionMode, setConnectionMode] = useState(() => {
    return localStorage.getItem("connectionMode") || "auto-connecting";
  });

  // Auto-manage connection mode based on WebSocket status
  // useEffect(() => {
  //   if (wsConnected) {
  //     // Successfully connected to hardware
  //     if (connectionMode !== "pacemaker") {
  //       console.log("🔌 Auto-switching to pacemaker mode (connected)");
  //       setConnectionMode("pacemaker");
  //       localStorage.setItem("connectionMode", "pacemaker");
  //     }
  //   } else {
  //     // Not connected to hardware - use simulation
  //     if (connectionMode !== "simulated" && connectionMode !== "auto-connecting") {
  //       console.log("📱 Auto-switching to simulation mode (disconnected)");
  ...
  useEffect(() => {
    if (wsConnected) {
      ...
      // Not connected to hardware - use simulation
      if (connectionMode === "pacemaker" || connectionMode === "auto-connecting") {
        console.log("📱 Auto-switching to simulation mode (disconnected)");
        setConnectionMode("simulated");
        localStorage.setItem("connectionMode", "simulated");
      }
    }
  }, [wsConnected]); // ONLY wsConnected - removed connectionMode to prevent loops


  const isConnected = connectionMode === "pacemaker" && wsConnected;
  const isSimulated = connectionMode === "simulated" || (!wsConnected && connectionMode !== "auto-connecting");
  ...
  // Sync WebSocket data with display when connected
  // useEffect(() => {
  //   if (isConnected && pacemakerState) {
  //     console.log('🔄 Syncing WebSocket data to display:', pacemakerState);
  //     setPacemakerParams({
  ...
  //     });
  //   }
  // }, [pacemakerState, isConnected]);

  useEffect(() => {
    // Only update from WebSocket if we're connected AND not in simulation mode
    // AND we're not currently adjusting sliders
    if (isConnected && wsConnected && pacemakerState && connectionMode === 'pacemaker') {
      console.log('🔄 Syncing WebSocket data to display:', pacemakerState);
      
      // Only update if values are actually different (prevents flashing)
      const newParams = {
        rate: pacemakerState.rate,
        aOutput: pacemakerState.a_output,
        vOutput: pacemakerState.v_output,
      ...
      setPacemakerParams(prev => {
        ...
        const hasChanged = Object.keys(newParams).some(key => 
          Math.abs(prev[key as keyof typeof prev] - newParams[key as keyof typeof newParams]) > 0.01
        );
  ...
  }, [pacemakerState, isConnected, wsConnected, connectionMode]); // Added connectionMode dependency
  ...
  useEffect(() => {
    if (currentModule && !currentSession && connectionMode !== 'pacemaker') {
      // Only reset if we're not connected to hardware and no active session
      const currentParams = JSON.stringify(pacemakerParams);
      const initialParams = JSON.stringify(currentModule.initialParams);

      if (currentParams !== initialParams) {
        console.log('🔄 Resetting to module initial parameters');
        setPacemakerParams(currentModule.initialParams);
      }
    }
  }, [moduleId, currentModule, currentSession?.id, connectionMode]); // Added connectionMode
  ...
  useEffect(() => {
    ...
    const leftShouldFlash =
      pacemakerParams.aOutput > 0 && pacemakerParams.aSensitivity > 0;
    const rightShouldFlash =
      pacemakerParams.vOutput > 0 && pacemakerParams.vSensitivity > 0;
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [currentSession?.id, currentSession?.completedAt, updateSession]); // Removed pacemakerParams
  ...
  //       // Map our parameter names to WebSocket parameter names
  ...
  const handleModuleParameterChange = useCallback(
    (param: string, value: number) => {
      ...
      // Send to hardware if connected (but don't wait for response)
  ...
  const getConnectionStatusDisplay = () => {
    if (connectionMode === "auto-connecting") {
      return {
        className: "bg-yellow-100 text-yellow-800",
        icon: <WifiOff className="w-4 h-4 mr-2 animate-pulse" />,
        text: "Connecting..."
      };
    } else if (isConnected) {
      return {
      ...
      <div className="w-full px-8 py-8 bg-white shadow-lg rounded-3xl">
        ...
        <div className="grid grid-cols-3 gap-8">
          ...
          <div className="col-span-2 space-y-6">
            ...
            {isConnected && quizCompleted && (
              <div className="bg-green-50 rounded-xl p-6 border-2 border-green-200">
                ...
                <p className="text-green-800 mt-3">
                  ✅ Use the physical pacemaker controls to adjust settings. Values update in real-time.
                </p>
              </div>
            )}
            ...
            {connectionMode === "auto-connecting" && (
              <div className="bg-yellow-50 rounded-xl p-6 border-2 border-yellow-200">
                ...
                <p className="text-yellow-800">
                  Attempting to connect to the pacemaker device. Will automatically switch to simulation mode if connection fails.
                </p>
              </div>
            )}
