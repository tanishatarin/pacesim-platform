...
//       console.log(
//         `🔧 Parameter changed: ${param} from ${oldValue} to ${value}`,
//       );
...
const ModulePage = () => {
  ...
  const [pacemakerParams, setPacemakerParams] = useState(currentModule.initialParams);
  const [quizCompleted, setQuizCompleted] = useState(false);
  const [quizPassed, setQuizPassed] = useState(false);
  const [quizScore, setQuizScore] = useState({ score: 0, total: 0 });
  const [showCompletion, setShowCompletion] = useState(false);
  ...
  useEffect(() => {
    ...
    console.log('📊 Resetting to module initial params:', currentModule.initialParams);
    setPacemakerParams(currentModule.initialParams);
  ...
  useEffect(() => {
    ...
    console.log('📺 DISPLAY PARAMS:', { displayRate, displayAOutput, displayVOutput });
  ...
  useEffect(() => {
    ...
    initializeModule();
  }, [currentUser?.id, moduleId, currentModule?.title, currentModule?.initialParams]);
