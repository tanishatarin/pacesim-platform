...
const ProfilePage = () => {
  ...
  const getDisplayedSessionGroups = () => {
    ...
    return sessionGroups.slice(0, 8);
  };
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-center">
              <div className="text-lg font-bold text-blue-600">
                {Math.round(sessionHistory.reduce((sum, s) => sum + (s.activeTimeSpent || 0), 0) / 60)}m
              </div>
            ...
            <div className="space-y-3">
              {sessionHistory
                ...
                .map((session) => {
                  ...
                  const hasActiveTime = session.activeTimeSpent && session.activeTimeSpent > 0;
                  const activeVsTotalRatio = session.activeTimeSpent && session.totalTimeSpent 
                    ? (session.activeTimeSpent / session.totalTimeSpent) * 100 
                    : 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-6 gap-3 text-xs">
                        ...
                        <div className={`p-2 rounded ${hasActiveTime ? 'bg-green-100' : 'bg-white'}`}>
                          <div className="text-gray-500">Active Time</div>
                          <div className="font-medium text-gray-900">
                            {session.activeTimeSpent ? formatDuration(session.activeTimeSpent, true) : "0s"}
                            {hasActiveTime && " 🎯"}
                          </div>
                        </div>
                        ...
                        <div className="p-2 bg-white rounded">
                          ...
                          <div className="font-medium text-gray-900">
                            {activeVsTotalRatio > 0 ? `${Math.round(activeVsTotalRatio)}%` : "N/A"}
                          </div>
                        </div>
                      ...
                      {/* Active Time Segments Debug */}
                      {session.practiceState?.timeSegments && session.practiceState.timeSegments.length > 0 && (
                        <div className="mt-3 p-2 bg-blue-50 rounded text-xs">
                          <details>
                            <summary className="font-medium text-blue-800 cursor-pointer">
                              Time Segments ({session.practiceState.timeSegments.length})
                            </summary>
                            <div className="mt-2 space-y-1">
                              {session.practiceState.timeSegments.slice(-3).map((segment, index) => (
                                <div key={index} className="text-blue-700">
                                  {segment.activity}: {segment.duration ? `${segment.duration}s` : 'ongoing'} 
                                  ({new Date(segment.startTime).toLocaleTimeString()})
                                </div>
                              ))}
                              {session.practiceState.timeSegments.length > 3 && (
                                <div className="text-blue-600">
                                  ... and {session.practiceState.timeSegments.length - 3} more segments
                                </div>
                              )}
                            </div>
                          </details>
                      ...
                      <div className="mt-3 space-y-1">
                        ...
                        {hasActiveTime && session.totalTimeSpent && activeVsTotalRatio < 50 && (
                          <div className="text-xs text-orange-700 bg-orange-100 px-2 py-1 rounded">
                            📱 Low engagement ratio ({Math.round(activeVsTotalRatio)}%) - user may have been away from page
                          </div>
                        )}
                        {!hasActiveTime && session.totalTimeSpent && session.totalTimeSpent > 60 && (
                          <div className="text-xs text-red-700 bg-red-100 px-2 py-1 rounded">
                            ❌ No active time recorded despite long session - tracking issue
                          </div>
                        )}
                      </div>
                    </div>
          ...
          {sessionHistory.length > 0 && (
            <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg">
              <h4 className="font-medium text-gray-800 mb-3">📊 Active Time Analysis</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <div className="text-gray-600">Total Active Time</div>
                  <div className="font-bold text-blue-600">
                    {formatDuration(sessionHistory.reduce((sum, s) => sum + (s.activeTimeSpent || 0), 0))}
                  </div>
                </div>
                <div>
                  <div className="text-gray-600">Average Efficiency</div>
                  <div className="font-bold text-green-600">
                    {Math.round(
                      sessionHistory
                        .filter(s => s.activeTimeSpent && s.totalTimeSpent)
                        .reduce((sum, s) => sum + ((s.activeTimeSpent! / s.totalTimeSpent!) * 100), 0) /
                      sessionHistory.filter(s => s.activeTimeSpent && s.totalTimeSpent).length || 0
                    )}%
                  </div>
                </div>
                <div>
                  <div className="text-gray-600">Sessions with Active Time</div>
                  <div className="font-bold text-purple-600">
                    {sessionHistory.filter(s => s.activeTimeSpent && s.activeTimeSpent > 0).length}/
                    {sessionHistory.length}
                  </div>
                </div>
                <div>
                  <div className="text-gray-600">Avg Active per Session</div>
                  <div className="font-bold text-orange-600">
                    {formatDuration(
                      sessionHistory.reduce((sum, s) => sum + (s.activeTimeSpent || 0), 0) / 
                  ...
                  <pre className="text-xs text-gray-600">
                    {JSON.stringify(
                      sessionHistory.slice(0, 3).map(s => ({
                        ...
                        activeTimeSpent: s.activeTimeSpent,
                        timeSegments: s.practiceState?.timeSegments?.length || 0,
