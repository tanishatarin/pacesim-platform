...
// above code is the debug version - super super helpful if you are developing! 
            ...
            <h3 className="text-lg font-semibold text-gray-900 flex items-center">
              ...
              <span className="ml-2 text-sm text-gray-500">
                ({sessionHistory.filter(s => 
                  Object.keys(REAL_MODULES).includes(s.moduleId) && 
                  s.totalTimeSpent && 
                  s.totalTimeSpent > 10
                ).length} sessions)
              </span>
          ...
          {sessionHistory.filter(s => 
            ...
            <div className="space-y-3">
              {sessionHistory
                ...
                .sort((a, b) => new Date(b.startedAt).getTime() - new Date(a.startedAt).getTime())
                .slice(0, showAllSessions ? undefined : 10)
                .map((session) => (
                  <div
                    key={session.id}
                    className={`border rounded-lg p-4 ${
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        ...
                        {!session.completedAt ? "In Progress" : session.isSuccess ? "Completed" : "Failed"}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        ...
                        <span className="ml-1 font-medium">
                          {formatDuration(session.totalTimeSpent || 0, true)}
                        </span>
                      </div>
                      ...
                      <div>
                        ...
                        <span className="ml-1 font-medium">{session.hintsUsed || 0}</span>
                      </div>
                      
                      <div>
                        <span className="text-gray-500">Parameter Changes:</span>
                        <span className="ml-1 font-medium">
                          {session.practiceState?.parameterChanges?.length || 0}
                        </span>
                      </div>
